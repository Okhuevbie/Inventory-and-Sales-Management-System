...
if ($getOrder) {
    ...
    $warehouseid = $orow['warehouse_id'];
    $name = $sl->getName('suppliers','supplier_name','id',$customer_name);
    $warehouseAddress = $sl->getName('stores','address','id',$warehouseid);
}else{
...
<html lang="en">
  ...
  <head>
    <title>Reciept | Warehouse System</title>
...
if ($getItems) {
  ...
  while ($row = $getItems->fetch_assoc()) {
      ...
      $itemno[$i] = $sl->getName('products','product_code','id',$row['product_price_id']);
      $mdesc[$i] = $sl->getName('products','product_name','id',$row['product_price_id']);
      $cat[$i] = $sl->getName('products','category','id',$row['product_price_id']);
      $category = $cat[$i];
      $cati[$i] = $sl->getName('product_categories','title','id',$category);
  ...
  ?>
    <body onLoad="window.opener.location.reload();">
        <table width="35%" style="border:1px solid #333333; background:url(); height:600px;" align="center">
          ...
          <td colspan="5"><div  align="center"><span ></span><i  class="style0"> <?php echo $orow['name'];?></i><br><i><?php echo $warehouseAddress; ?></i></div></td>
          </tr>
        <tr>
          
          <td colspan="2" rowspan="2">
            <div align="left" class="style1 "><span class="style3">Customer Name:</span><i> <?php echo $name;?></i></div>    
        ...
        <td height="29" style="border:1px solid #666666;"><strong></strong></td>
        ...
        <tr style="border:1px dashed #666666">
          <td colspan="4" style="border-bottom:2px double #666666; font-size:12px; font-weight:bold; background-color:#FFFFFF;border:1px solid #cccccc;">Total Sales </td>
          <td colspan="3" style="border-bottom:2px double #666666; font-size:15px; font-weight:bold; background-color:#FFFFFF; border:1px solid #cccccc;" valign='top'><?php
          $total = $orow['total'];
        ...
        <tr style="border:1px dashed #666666">
          <td height="26" colspan="4" style="border-bottom:2px double #666666; font-size:12px; font-weight:bold; background-color:#FFFFFF;border:1px solid #cccccc;">Discount</td>
          <td colspan="3" style="border-bottom:2px double #666666; font-size:15px; font-weight:bold; background-color:#FFFFFF; border:1px solid #cccccc;" valign='top'><?php echo "NGN ".number_format($orow['discount'], 2); ?></td>
        </tr>
        <tr style="border:1px dashed #666666">
          <td colspan="4" style="border-bottom:2px double #666666; font-size:12px; font-weight:bold; background-color:#FFFFFF;border:1px solid #cccccc;">Amount Paid</td>
          <td colspan="3" style="border-bottom:2px double #666666; font-size:15px; font-weight:bold; background-color:#FFFFFF; border:1px solid #cccccc;" valign='top'><?php
        ...
        <tr style="border:1px dashed #666666"><td colspan="4" style="border-bottom:2px double #666666; font-size:18px; font-weight:bold; background-color:#FFFFFF;border:1px solid #cccccc;"><span style="border-bottom:2px double #666666; font-size:12px; font-weight:bold; background-color:#FFFFFF;border:0px solid #cccccc;">Balance Due</span></td>
          <td colspan="3" style="border-bottom:2px double #666666; font-size:15px; font-weight:bold; background-color:#FFFFFF; border:1px solid #cccccc;"><?php 
          $balance = $total - $paid;
          echo "NGN ".number_format($balance, 2);
          ?></td>
        </tr>
        <tr style="border:1px dashed #666666">
          <td height="64" colspan="6" style="border-bottom:2px double #666666; font-size:18px; font-weight:bold; background-color:#FFFFFF;border:1px solid #cccccc;" valign="bottom"><span class="style2">Customer Sign</span>______________________ <span class="style2">Phone: <?php 
          $phone_no = $sl->getName('suppliers','phone','id',$customer_name);
          echo $phone_no ?></span></td>
          </tr>
        <tr style="border:1px dashed #666666">
          <td height="30" colspan="4" style="border-bottom:0px double #666666; font-size:18px; font-weight:bold; background-color:#FFFFFF;border:1px solid #cccccc;"><span class="style2">Thanks for your Patronage. Items Bought in Good Conditions are not Returnable</span></td>
          <td colspan="3" style="border-bottom:2px double #666666; font-size:18px; font-weight:bold; background-color:#FFFFFF;border:0px solid #cccccc;"><span class="style2">Cashier: <?php 
          $user = Session::get('user_fullname');
          echo $user;?></span></td>
        </tr>
        <tr style="border:1px dashed #666666">
          <td height="12" colspan="6" align="center" style="border-bottom:0px double #666666; font-size:14px; font-weight:bold; background-color:#333333;color:#ffffff;border:1px solid #cccccc;">Software By: Okhuevbie J. Toluwani and Richard R. Pam.<br><i class="fa fa-phone"></i> 08138450009 || 09071482526</td>
          </tr>
          <?php 
          $uid = USERID;
